...
def generate_per_id():
    """회원가입 당시의 년월일시분초를 기반으로 고유 per_id 생성"""
    kst = pytz.timezone('Asia/Seoul')
    now = datetime.now(kst)
    return 'PERS' + now.strftime('%Y%m%d%H%M%S')

# 사용자 테이블
class User(db.Model, UserMixin):
    per_id = db.Column(db.String(20), primary_key=True, default=generate_per_id)  # 사용자 고유 ID
...
# 게시글 테이블
class Post(db.Model):
    per_id = db.Column(db.String(20), db.ForeignKey('user.per_id'), primary_key=True)  # 게시글 작성자의 per_id
...
class Comment(db.Model):
    per_id = db.Column(db.String(20), db.ForeignKey('user.per_id'), primary_key=True)  # 댓글 작성자의 per_id
    post_id = db.Column(db.String(20), db.ForeignKey('post.per_id'), nullable=False)      # 게시글 (Post 테이블 참조)
...
class BoardType(db.Model):
    per_id = db.Column(db.String(20), primary_key=True)                        # 게시판 유형 ID
...
class UserLoginFail(db.Model):
    per_id = db.Column(db.String(20), db.ForeignKey('user.per_id'),  primary_key=True)                        # 고유 ID
    user_id = db.Column(db.String(50), db.ForeignKey('user.user_id'), nullable=False)  # 실패한 사용자 ID (User 테이블 참조)
