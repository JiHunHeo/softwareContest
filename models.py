...
class User(db.Model, UserMixin):
    ...
    user_id = db.Column(db.String(50), nullable=True, unique=True)                  # 로그인용 아이디 (선택적)
    ...
    birthdate = db.Column(db.Date, nullable=True)                                  # 생년월일 (선택적)
    student_id = db.Column(db.String(20), nullable=True)              # 학번 (선택적)
    phone_number = db.Column(db.String(15), nullable=True)            # 휴대폰 번호 (선택적)
    address = db.Column(db.String(255), nullable=True)                             # 주소 (선택적)
    is_employed = db.Column(db.Boolean, default=False)                              # 직장인 여부
    security_question = db.Column(db.String(255), nullable=True)                   # 비밀번호 찾기 질문 (선택적)
    security_answer = db.Column(db.String(255), nullable=True)                     # 비밀번호 찾기 답변 (선택적)
...
# 게시글 테이블
class Post(db.Model):
    per_id = db.Column(db.String(24), primary_key=True, default=generate_per_id)               # 게시글 고유 ID
    author_id = db.Column(db.String(24), db.ForeignKey('user.per_id'), nullable=False)        # 게시글 작성자의 per_id
    title = db.Column(db.String(100), nullable=False)                                               # 게시글 제목
    content = db.Column(db.Text, nullable=False)                                                    # 게시글 내용
    board_type = db.Column(db.String(50), default='일반')                                           # 게시판 유형 (간단한 문자열)
    board_type_id = db.Column(db.String(20), db.ForeignKey('board_type.per_id'), nullable=True)    # 게시판 유형 (BoardType 참조) - 선택적
...
class Comment(db.Model):
    per_id = db.Column(db.String(24), primary_key=True, default=generate_per_id)    # 댓글 고유 ID
    author_id = db.Column(db.String(24), db.ForeignKey('user.per_id'), nullable=False)   # 댓글 작성자의 per_id
...
class BoardType(db.Model):
    per_id = db.Column(db.String(24), primary_key=True, default=generate_per_id)             # 게시판 유형 ID
